...
@Component({
  selector: 'app-students-teachers-info-jigyaasa',
  templateUrl: './students-teachers-info-jigyaasa.component.html',
  styleUrls: ['./students-teachers-info-jigyaasa.component.scss']
})
export class StudentsTeachersInfoJigyaasaComponent implements OnInit {
 teacherInfoUi: boolean = false;
  studentInfo: any;
  teacherInfo: any;
  _unsubscribeAll: Subject<any> = new Subject<any>();
  filteredStudentInfo: any;
  test = [];
  params: any;
  filteredTecherInfo: any[];
  ;
  selectedClsId: any;
  ...
  async ngOnInit(): Promise<void> {
    this.drawerService.drawerOpenSubject.subscribe((res) => {
      if (res) {
        ...
        this.studentCompletionPercentage = {};
    ...
    // this.drawerService.drawerOpenLearningUnitsSubject.subscribe(res => {
    //   if (res) {
    //     this.getAllStudents()
    //     this.getTeacher()
    ...
    //       // res.accessLevel >= 10 ? this.studentsAuthInfoUi = false : this.studentsAuthInfoUi = true
    //     })
    //     const learningUnitArray: any = res
    //     this.learningUnitArray = learningUnitArray.sort((a: any, b: any) => a?.docId?.localeCompare(b?.docId));
    //   }

    // })

    // this.drawerService.drawerOpenWorkflowsSubject.subscribe(res => {
    //   if (res) {
    //     const workflowsArray: any = res
    //     this.workflowsArray = workflowsArray.sort((a: any, b: any) => a?.learningUnitId?.localeCompare(b?.learningUnitId));
    //     this.noWorkflowTooltipMessage = `This learning unit has not been configured yet. Please contact us at +${environment.whatsAppSender} or at ${environment.emailSender} for further assistance.`;
    //   }
    // })
    ...
    //   // res.accessLevel >= 10 ? this.studentsAuthInfoUi = false : this.studentsAuthInfoUi = true
    // })
    // const learningUnitArray = this.drawerService.drawerOpenLearningUnitsSubject.value as unknown as Array<any>;
    // this.learningUnitArray = learningUnitArray.sort((a: any, b: any) => a?.docId?.localeCompare(b?.docId));
    // console.log(this.learningUnitArray);

    // const workflowsArray = this.drawerService.drawerOpenWorkflowsSubject.value as unknown as Array<any>;
    // this.workflowsArray = workflowsArray.sort((a: any, b: any) => a?.learningUnitId?.localeCompare(b?.learningUnitId));
    // this.noWorkflowTooltipMessage = `This learning unit has not been configured yet. Please contact us at +${environment.whatsAppSender} or at ${environment.emailSender} for further assistance.`;
  }
  ...
  getAllStudents() {
    this.route.queryParams.pipe(first()).subscribe(async (param) => {
      const classroomId = param?.classroomId;
      this.selectedClsId = classroomId;
      this.selectedProgrammeId = param?.programmeId;
      this.selectedinstitutionId = param?.institutionId;
      const query: QueryFn = (ref: CollectionReference) =>
        ref.where(`classrooms.${classroomId}.classroomId`, '==', classroomId)
          .where('loginType', '==', 'userid_password');
      this.studentService.getWithQuery(query).pipe(first()).subscribe(async (res) => {
        const infos = this.querySelectedProgramFromResponse(res);
        this.getCompletePercentage(res, false);
        ...
        this.studentInfo=this.studentInfo.sort((a: any, b: any) => a?.authInfo?.accessCode - b?.authInfo?.accessCode);
        this.filteredStudentInfo = [...this.studentInfo];
      });
    });
  }
  ...
  async getTeacher() {
    this.route.queryParams.subscribe(async (param) => {
      ...
      this.teacherService.getWithQuery(query).pipe(first()).subscribe((res) => {
        this.teacherInfo = this.querySelectedProgramFromResponse(res);
        this.filteredTecherInfo=[...this.teacherInfo];
        this.getCompletePercentage(res, true);
      });
    });
  }
  ...
  async getCompletePercentage(res: any, isTeacher: boolean) {
    ...
    if (isTeacher) {
      ...
      await Promise.all(res.map(async (student: any, sIndex: number) => {
        ...
        this.studentCompletionPercentage[sIndex] = [];

        await Promise.all(this.workflowsArray.map(async (workflow: any, i: number) => {
          const { workflowId, learningUnitId } = workflow;
          this.studentCompletionPercentage[sIndex][i] = {};

          if (!workflowId || !learningUnitId) {
            this.studentCompletionPercentage[sIndex][i].workflowCompletion = 'No Workflow';
            console.error('Blank workflowId or learningUnitId found in programme');
          };
          ...
          const studentCompletionPercentage = await lastValueFrom(
            this.workflowCompletionService.getStudentCompletionById(docId, learningUnitId)
          );
          if (studentCompletionPercentage.exists) {
            const studentData: any = studentCompletionPercentage.data();
            ...
            if (totalWorkflowSteps == -1) {
              this.studentCompletionPercentage[sIndex][i].workflowCompletion = 'No Workflow';
              this.studentCompletionPercentage[sIndex][i].workflowId = '';
              return;
            }
            const studentCompletedPercentage = Math.round((studentCompletedSteps * 100) / totalWorkflowSteps);

            this.studentCompletionPercentage[sIndex][i].workflowCompletion = `${studentCompletedPercentage}%`;
            this.studentCompletionPercentage[sIndex][i].workflowId = workflowId;
          } else {
            if (totalWorkflowSteps == -1) {
              this.studentCompletionPercentage[sIndex][i].workflowCompletion = 'No Workflow';
              this.studentCompletionPercentage[sIndex][i].workflowId = '';
            ...
            this.studentCompletionPercentage[sIndex][i].workflowCompletion = '0%';
            this.studentCompletionPercentage[sIndex][i].workflowId = workflowId;
          };
          this.studentCompletionPercentage[sIndex][i].learningUnit = selectedLearningUnit;
  ...
  filterByQueryInStudentDoc(query: string) {
    ...
    if (lowerCaseQuery) {
      this.studentInfo = this.filteredStudentInfo.filter((student: any) =>
      ...
      this.studentInfo = this.filteredStudentInfo;
    }
  }

  filterByQueryInTeacherDoc(query: string) {
    // Convert query to lower case for case-insensitive comparison
    const lowerCaseQuery = query.toLowerCase();

    if (lowerCaseQuery) {
      this.teacherInfo = this.filteredTecherInfo.filter((student: any) =>
        // Check if authInfo exists and then filter based on the query
         (student?.teacherMeta?.firstName?.toLowerCase().includes(lowerCaseQuery) ||
          student?.teacherMeta?.lastName?.toLowerCase().includes(lowerCaseQuery) ||
          student?.teacherMeta?.phoneNumber?.toLowerCase().includes(lowerCaseQuery) ||
          student?.teacherMeta?.email?.toLowerCase().includes(lowerCaseQuery)
        )
      );
    } else {
      // If no query, show all students
      this.teacherInfo = this.filteredTecherInfo;
    }
  }
