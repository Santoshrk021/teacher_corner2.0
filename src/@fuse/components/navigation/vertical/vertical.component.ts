...
export class FuseVerticalNavigationComponent implements OnChanges, OnInit, AfterViewInit, OnDestroy {
    ...
    sortedNavigation: FuseNavigationItem[] = [];
    ...
    ngOnChanges(changes: SimpleChanges): void {
        ...
        if (changes['navigation'] && this.navigation) {
            this.sortedNavigation = this.sortNavigationGroups([...this.navigation]);
        }
        // Appearance
    ...
    private sortNavigationGroups(nav: FuseNavigationItem[]): FuseNavigationItem[] {
        return nav.filter(item => !!item).map(group => {
            const shouldSort =
                group?.type === 'group' &&
                ['Contests and Challenges', 'Nominations'].includes(group?.title) &&
