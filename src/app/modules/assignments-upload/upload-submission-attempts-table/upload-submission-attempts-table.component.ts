...
import { UploadSubmissionDialogComponent } from '../upload-submission-dialog/upload-submission-dialog.component';

@Component({
  selector: 'app-upload-submission-attempts-table',
  templateUrl: './upload-submission-attempts-table.component.html',
  styleUrls: ['./upload-submission-attempts-table.component.scss']
})
export class UploadSubmissionAttemptsTableComponent implements OnInit {
  params: any;
  ...
  goBack() {
    ...
    if (workflowId !== undefined) {
      this.router.navigate(['dashboard', classroomId, 'upload-submissions'], { queryParams: { institutionId, classroomId, programmeId, assignmentId, workflowId } });
    } else {
      this.router.navigate(['dashboard', classroomId, 'upload-submissions'], { queryParams: { institutionId, classroomId, programmeId, assignmentId } });
    };
  }
  ...
  async viewScore(assignmentDoc: any, selectedSubmission: any, index: number) {
    ...
    await import('../upload-submission-dialog/upload-submission-dialog.module');
    this.dialog.open(UploadSubmissionDialogComponent, {
      data: {
        quizInfo: {
          ...assignmentDoc.value,
  ...
  async uploadReplayDialog(assignmentDoc: any, selectedSubmission: any) {
    await import('../upload-replay/upload-replay.module');
