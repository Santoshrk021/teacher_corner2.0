...
@Component({
  selector: 'app-assignments-quiz',
  templateUrl: './assignments-quiz.component.html',
  styleUrls: ['./assignments-quiz.component.scss']
})
export class AssignmentsQuizComponent implements OnInit {
  infoArr: Array<any> = [];
  params: any;
  ...
  async getAllAssignments() {
    if (this.assignmentService.assignmentsSub.value !== null && this.assignmentService.assignmentsSub.value.length > 0) {
  ...
  goToSubmissionAttemptsTable(assignment: any) {
    ...
    if (workflowId !== undefined) {
      this.router.navigate(['dashboard', classroomId, 'quiz-submissions', studentId], { queryParams: { institutionId, classroomId, programmeId, assignmentId, workflowId } });
    } else {
      this.router.navigate(['dashboard', classroomId, 'quiz-submissions', studentId], { queryParams: { institutionId, classroomId, programmeId, assignmentId } });
    }
  }
