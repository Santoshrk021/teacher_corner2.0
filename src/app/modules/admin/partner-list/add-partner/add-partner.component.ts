...
export class AddPartnerComponent implements OnInit {
    ...
    async setForm() {
        ...
        const latestDocId = await this.ds.getLatestMasterDoc('PARTNER');
        this.infoForm = this.fb.group({
            docId: docId,
            pointOfContactCountryCode: [{ value: '+91', disabled: true }, [Validators.required]],
            organizationName: [{ value: '', disabled: false }, [Validators.required]],
            ...
            oragnizationphoneNumber: [{ value: '', disabled: true }, [Validators.required]],
            ...
            //partnerAddress: [{ value: this.data.address || '', disabled: true }, [Validators.required]],
            pointOfContactFirstName: [{ value: '', disabled: true }, [Validators.required]],
            pointOfContactLastname: [{ value: '', disabled: true }, [Validators.required]],
            ...
            partnerGpsCoordinates: [{ value: this.currentGpsCoordinates || '', disabled: true }, [Validators.required]],
        ...
        const watchList = [
            ...
            'partnerAddress.route',
            'partnerAddress.street',
            'partnerAddress.village',
            'partnerAddress.sublocality',
            'partnerAddress.city',
            'partnerAddress.subDistrict',
            'partnerAddress.district',
    ...
    async onSubmit(form) {
        ...
        value['pointOfContactFullName'] = value?.pointOfContactFirstName + ' ' + value?.pointOfContactLastname;
        value['oragnizationphoneNumber'] = value?.pointOfContactCountryCode + value?.oragnizationphoneNumber;
        value['Catagory'] = 'Partner';
        value['verificationStatus'] = false;
        value['isVerified'] = false;
        // Upsert to Master map first to get/ensure masterDocId
        const masterDocId = await this.ds.addNewObjectToMasterMap('PARTNER', 'partners', value);
        value['masterDocId'] = masterDocId || value['masterDocId'] || '';
        // Persist to partners collection with the same docId
        await this.partnerService.saveinPartnerColl(value, value.docId);
